...
const ACTIONS: Record<string, { desc: string; required?: string[]; handler: ActionHandler }> = {
  ...
  inviteGuestToWorkspace: {
    desc: "Invite a guest to the workspace. Args: email(required), can_edit_tags(optional bool), can_see_time_spent(optional bool), can_see_time_estimated(optional bool), can_create_views(optional bool), custom_role_id(optional). Enterprise only.",
    required: ["email"],
    handler: async (args, config) => {
      const body: Record<string, unknown> = { email: args.email };
      if (args.can_edit_tags != null) body.can_edit_tags = Boolean(args.can_edit_tags);
      if (args.can_see_time_spent != null) body.can_see_time_spent = Boolean(args.can_see_time_spent);
      if (args.can_see_time_estimated != null) body.can_see_time_estimated = Boolean(args.can_see_time_estimated);
      if (args.can_create_views != null) body.can_create_views = Boolean(args.can_create_views);
      if (args.custom_role_id != null) body.custom_role_id = Number(args.custom_role_id);

      const data = (await clickupFetch(`/team/${config.teamId}/guest`, config.apiKey, {
        method: "POST",
        body: JSON.stringify(body),
      })) as Record<string, unknown>;
      const guest = (data.team as Record<string, unknown>) || data;
      return `✅ Guest invited: ${args.email}\n${JSON.stringify(guest, null, 2)}`;
    },
  },

  getGuest: {
    desc: "Get guest details. Args: guestId(required)",
    required: ["guestId"],
    handler: async (args, config) => {
      const data = (await clickupFetch(
        `/team/${config.teamId}/guest/${args.guestId}`,
        config.apiKey,
      )) as Record<string, unknown>;
      const guest = (data.guest as Record<string, unknown>) || data;
      const email = (guest.email as string) || "—";
      const username = (guest.username as string) || "—";
      return `👤 Guest: ${username} (${email}) — ID: ${args.guestId}\n${JSON.stringify(guest, null, 2)}`;
    },
  },

  editGuest: {
    desc: "Edit guest permissions on workspace. Args: guestId(required), can_edit_tags(optional bool), can_see_time_spent(optional bool), can_see_time_estimated(optional bool), can_create_views(optional bool), custom_role_id(optional). Enterprise only.",
    required: ["guestId"],
    handler: async (args, config) => {
      const body: Record<string, unknown> = {};
      if (args.can_edit_tags != null) body.can_edit_tags = Boolean(args.can_edit_tags);
      ...
      const data = (await clickupFetch(`/team/${config.teamId}/guest/${args.guestId}`, config.apiKey, {
        method: "PUT",
        body: JSON.stringify(body),
      })) as Record<string, unknown>;
      return `✅ Guest ${args.guestId} updated\n${JSON.stringify(data, null, 2)}`;
    },
  },

  removeGuestFromWorkspace: {
    desc: "Remove a guest from the workspace. Args: guestId(required). Enterprise only.",
    required: ["guestId"],
    handler: async (args, config) => {
      await clickupFetch(`/team/${config.teamId}/guest/${args.guestId}`, config.apiKey, {
        method: "DELETE",
      });
      return `🗑️ Guest ${args.guestId} removed from workspace`;
    },
  },

  addGuestToTask: {
    desc: "Share a task with a guest. Args: taskId(required), guestId(required), permission_level(required: 'read'|'comment'|'edit'|'create'). Enterprise only.",
    required: ["taskId", "guestId", "permission_level"],
    handler: async (args, config) => {
      const data = (await clickupFetch(
        `/task/${args.taskId}/guest/${args.guestId}`,
        config.apiKey,
        {
          method: "POST",
          body: JSON.stringify({ permission_level: args.permission_level }),
        },
      )) as Record<string, unknown>;
      return `✅ Guest ${args.guestId} added to task ${args.taskId} (${args.permission_level})\n${JSON.stringify(data, null, 2)}`;
    },
  },

  removeGuestFromTask: {
    desc: "Remove a guest from a task. Args: taskId(required), guestId(required). Enterprise only.",
    required: ["taskId", "guestId"],
    handler: async (args, config) => {
      await clickupFetch(`/task/${args.taskId}/guest/${args.guestId}`, config.apiKey, {
        method: "DELETE",
      });
      return `🗑️ Guest ${args.guestId} removed from task ${args.taskId}`;
    },
  },

  addGuestToList: {
    desc: "Share a list with a guest. Args: listId(required), guestId(required), permission_level(required: 'read'|'comment'|'edit'|'create'). Enterprise only.",
    required: ["listId", "guestId", "permission_level"],
    handler: async (args, config) => {
      const data = (await clickupFetch(
        `/list/${args.listId}/guest/${args.guestId}`,
        config.apiKey,
        {
          method: "POST",
          body: JSON.stringify({ permission_level: args.permission_level }),
      ...
      return `✅ Guest ${args.guestId} added to list ${args.listId} (${args.permission_level})\n${JSON.stringify(data, null, 2)}`;
    },
  },

  removeGuestFromList: {
    desc: "Remove a guest from a list. Args: listId(required), guestId(required). Enterprise only.",
    required: ["listId", "guestId"],
    handler: async (args, config) => {
      await clickupFetch(`/list/${args.listId}/guest/${args.guestId}`, config.apiKey, {
        method: "DELETE",
      });
      return `🗑️ Guest ${args.guestId} removed from list ${args.listId}`;
    },
  },

  addGuestToFolder: {
    desc: "Share a folder with a guest. Args: folderId(required), guestId(required), permission_level(required: 'read'|'comment'|'edit'|'create'). Enterprise only.",
    required: ["folderId", "guestId", "permission_level"],
    handler: async (args, config) => {
      const data = (await clickupFetch(
        `/folder/${args.folderId}/guest/${args.guestId}`,
        config.apiKey,
        {
          method: "POST",
          body: JSON.stringify({ permission_level: args.permission_level }),
      ...
      return `✅ Guest ${args.guestId} added to folder ${args.folderId} (${args.permission_level})\n${JSON.stringify(data, null, 2)}`;
    },
  },

  removeGuestFromFolder: {
    desc: "Remove a guest from a folder. Args: folderId(required), guestId(required). Enterprise only.",
    required: ["folderId", "guestId"],
    handler: async (args, config) => {
      await clickupFetch(`/folder/${args.folderId}/guest/${args.guestId}`, config.apiKey, {
        method: "DELETE",
      });
      return `🗑️ Guest ${args.guestId} removed from folder ${args.folderId}`;
    },
  },
  ...
  inviteUserToWorkspace: {
    desc: "Invite a user (member) to the workspace. Args: email(required), admin(optional bool), custom_role_id(optional)",
    required: ["email"],
    handler: async (args, config) => {
      const body: Record<string, unknown> = { email: args.email };
      if (args.admin != null) body.admin = Boolean(args.admin);
      if (args.custom_role_id != null) body.custom_role_id = Number(args.custom_role_id);

      const data = (await clickupFetch(`/team/${config.teamId}/user`, config.apiKey, {
        method: "POST",
        body: JSON.stringify(body),
      })) as Record<string, unknown>;
      return `✅ User invited: ${args.email}\n${JSON.stringify(data, null, 2)}`;
    },
  },

  getUser: {
    desc: "Get user details. Args: userId(required)",
    required: ["userId"],
    handler: async (args, config) => {
      const data = (await clickupFetch(
        `/team/${config.teamId}/user/${args.userId}`,
        config.apiKey,
      )) as Record<string, unknown>;
      const user = (data.member as Record<string, unknown>) || (data.user as Record<string, unknown>) || data;
      return `👤 User ${args.userId}\n${JSON.stringify(user, null, 2)}`;
    },
  },

  editUser: {
    desc: "Edit user on workspace. Args: userId(required), admin(optional bool), custom_role_id(optional)",
    required: ["userId"],
    handler: async (args, config) => {
      const body: Record<string, unknown> = {};
      if (args.admin != null) body.admin = Boolean(args.admin);
      ...
      const data = (await clickupFetch(`/team/${config.teamId}/user/${args.userId}`, config.apiKey, {
        method: "PUT",
        body: JSON.stringify(body),
      })) as Record<string, unknown>;
  ...
  removeUserFromWorkspace: {
    desc: "Remove a user from the workspace. Args: userId(required)",
    required: ["userId"],
    handler: async (args, config) => {
      await clickupFetch(`/team/${config.teamId}/user/${args.userId}`, config.apiKey, {
        method: "DELETE",
      });
      return `🗑️ User ${args.userId} removed from workspace`;
    },
  },
  ...
  getTaskMembers: {
    desc: "Get members with access to a task. Args: taskId(required)",
    required: ["taskId"],
    handler: async (args, config) => {
      const data = (await clickupFetch(`/task/${args.taskId}/member`, config.apiKey)) as Record<
        string,
        unknown
      >;
      const members = (data.members as Array<Record<string, unknown>>) || [];
      if (!members.length) return "No members found for this task.";
      return `👥 Task Members (${members.length})\n${members.map((m) => `• ${m.username} (${m.email}) — ID: ${m.id}`).join("\n")}`;
    },
  },
  ...
  getCustomRoles: {
    desc: "Get custom roles defined in the workspace.",
    handler: async (_args, config) => {
      const data = (await clickupFetch(
        `/team/${config.teamId}/customroles`,
        config.apiKey,
      )) as Record<string, unknown>;
      const roles = (data.custom_roles as Array<Record<string, unknown>>) || [];
      if (!roles.length) return "No custom roles defined.";
      return `🎭 Custom Roles (${roles.length})\n${roles.map((r) => `• ${r.name} — ID: ${r.id}`).join("\n")}`;
    },
  },
  ...
  getSharedHierarchy: {
    desc: "Get all resources shared with the authenticated user (spaces, folders, lists, tasks).",
    handler: async (_args, config) => {
      const data = (await clickupFetch(
        `/team/${config.teamId}/shared`,
        config.apiKey,
      )) as Record<string, unknown>;
      const shared = data.shared as Record<string, unknown> | undefined;
      if (!shared) return JSON.stringify(data, null, 2);
      const parts: string[] = [];
      for (const [key, items] of Object.entries(shared)) {
        const arr = items as Array<Record<string, unknown>>;
        if (arr?.length) {
          parts.push(`${key} (${arr.length}):\n${arr.map((i) => `  • ${i.name || i.id}`).join("\n")}`);
        }
      }
      return parts.length ? `🔗 Shared Hierarchy\n\n${parts.join("\n\n")}` : "Nothing shared.";
    },
  },
  ...
  getList: {
    desc: "Get details of a single list. Args: listId(required)",
    required: ["listId"],
    handler: async (args, config) => {
      const data = (await clickupFetch(`/list/${args.listId}`, config.apiKey)) as Record<
        string,
        unknown
      >;
      const name = (data.name as string) || "Untitled";
      const taskCount = (data.task_count as number) || 0;
      const status = ((data.status as Record<string, unknown>)?.status as string) || "—";
      const space = (data.space as Record<string, unknown>) || {};
      const folder = (data.folder as Record<string, unknown>) || {};
      let out = `📋 ${name}\n`;
      out += `ID: ${data.id} | Tasks: ${taskCount} | Status: ${status}\n`;
      out += `Space: ${space.name || "—"} | Folder: ${folder.name || "(folderless)"}`;
  ...
  updateList: {
    desc: "Update a list. Args: listId(required), name(optional), content(optional description), due_date(optional timestamp ms), due_date_time(optional bool), priority(optional 1-4), assignee(optional userId to add), unset_status(optional bool)",
    required: ["listId"],
    handler: async (args, config) => {
      const { listId, ...fields } = args;
      const body: Record<string, unknown> = {};
      if (fields.name) body.name = fields.name;
      ...
      if (fields.due_date_time != null) body.due_date_time = Boolean(fields.due_date_time);
      if (fields.priority != null) body.priority = Number(fields.priority);
      if (fields.assignee != null) body.assignee = Number(fields.assignee);
      if (fields.unset_status != null) body.unset_status = Boolean(fields.unset_status);

      const data = (await clickupFetch(`/list/${listId}`, config.apiKey, {
        method: "PUT",
        body: JSON.stringify(body),
      })) as Record<string, unknown>;
      return `✅ List updated: [${data.id}] ${data.name}`;
    },
  },

  // ── Privacy & Access (API v3) ────────────────────────────────────────────

  updateAccessControl: {
    desc: "Update privacy and access of an object. Args: objectType(required: 'space'|'folder'|'list'|'task'), objectId(required), body(required: raw JSON object to send — e.g. {\"is_private\": true, \"members\": [{\"id\": 123, \"permission_level\": \"edit\"}]}). Uses API v3.",
    required: ["objectType", "objectId", "body"],
    handler: async (args, config) => {
      const objectType = String(args.objectType);
      const objectId = String(args.objectId);
      const body = typeof args.body === "string" ? JSON.parse(args.body as string) : args.body;

      const data = (await clickupFetchV3(
        `/workspaces/${config.teamId}/${objectType}/${objectId}/acls`,
        config.apiKey,
        {
          method: "PATCH",
          body: JSON.stringify(body),
      ...
      return `✅ Access updated for ${objectType} ${objectId}\n${JSON.stringify(data, null, 2)}`;
    },
  },
