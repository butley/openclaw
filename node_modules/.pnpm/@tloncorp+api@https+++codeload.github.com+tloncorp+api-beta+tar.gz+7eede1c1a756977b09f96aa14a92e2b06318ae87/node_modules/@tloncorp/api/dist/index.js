...
(function(e, t) {
  (function() {
    ...
    var t1 = function U(G) {
      ...
      (ac && Jt(new ac(new ArrayBuffer(1))) != h || Gs && Jt(new Gs()) != Ct || uc && Jt(uc.resolve()) != Fi || is && Jt(new is()) != It || Ws && Jt(new Ws()) != ut) && (Jt = function(i) {
...
`);
      ...
      function Wv(i, a) {
        ...
        return O = a[5], O && (M = i[5], i[5] = M ? xp(M, O, a[6]) : O, i[6] = M ? si(i[5], m) : a[6]), O = a[7], O && (i[7] = O), D & De && (i[8] = i[8] == null ? a[8] : Zt(i[8], a[8])), i[9] == null && (i[9] = a[9]), i[0] = a[0], i[1] = b, i;
      }
      function Kv(i) {
        var a = [];
        if (i != null)
      ...
      var NC = Ev(function(i, a) {
        ...
        return Te(function(D) {
          for (var b = -1, F = Zt(D.length, l); ++b < F; )
      ...
      function mg(i) {
        ...
        if (js && i[js])
          return KE(i[js]());
        var a = Jt(i), l = a == Ct ? ic : a == It ? Vo : fs;
        return l(i);
      }
      function zr(i) {
        if (!i)
  ...
  var t = function(n) {
    ...
    function Ls(S, g) {
      ...
      for (j.length <= h && j.push(0), ae.push(0), I = ae[p - 1], ce = h - p; ce >= 0; ce--) {
        for (de = v - 1, j[ce + p] !== I && (de = Math.floor((j[ce + p] * v + j[ce + p - 1]) / I)), me = 0, sn = 0, Ji = ae.length, Vt = 0; Vt < Ji; Vt++)
          me += de * ae[Vt], Xi = Math.floor(me / v), sn += j[ce + Vt] - (me - Xi * v), me = Xi, sn < 0 ? (j[ce + Vt] = sn + v, sn = -1) : (j[ce + Vt] = sn, sn = 0);
        for (; sn !== 0; ) {
          for (de -= 1, me = 0, Vt = 0; Vt < Ji; Vt++)
            me += j[ce + Vt] - v + ae[Vt], me < 0 ? (j[ce + Vt] = me + v, me = 0) : (j[ce + Vt] = me, me = 1);
          sn += me;
        }
        C[ce] = de;
      }
...
function nd(...e) {
  return "/" + e.filter((t) => t != null).map((t) => typeof t == "number" ? Wt("ud", BigInt(t)) : t).join("/");
}
function Vg(e) {
  return e[0] === "#";
}
...
function a3(e) {
  return Wt("ud", ur.fromUnix(e.getTime()));
}
function ar(e) {
  return e.startsWith("~");
}
...
function je(e) {
  let t = e;
  return t[0] === "~" && (t = t.split("/").pop()), t[3] !== "." && (t = Wt("ud", BigInt(t))), t;
}
function Zy({
  content: e,
  authorId: t,
...
function E3(e) {
  return Wt("t", Y.fromCord(e).number);
}
function v3(e) {
...
const iB = (e) => {
  ...
  if (n)
    s = await Ve({
      app: "channels",
      path: `/${e.channelId}/search/bounded/text/${e.cursor ? Wt("ud", BigInt(e.cursor ?? 0)) : ""}/500/${r}`
    });
  else {
    const c = rd(e.channelId) === "dm" ? "dm" : "club";
    s = await Ve({
      app: "chat",
      path: `/${c}/${e.channelId}/search/bounded/text/${e.cursor ? Wt("ud", BigInt(e.cursor ?? 0)) : ""}/500/${r}`
    });
  }
...
function i0(e) {
  ...
  const o = Wt("p", s.number), c = r.at(Y.fromInt(14));
  if (!c)
...
function SB(e) {
  ...
  const r = Wt("p", n.head.number), s = Bs.cord(n.tail);
...
function CB(e) {
  ...
  const r = Wt("p", n.head.number), s = n.tail;
  if (!(s instanceof ze))
...
const nn = Cn("lanyardApi", !1);
function YU(e) {
  Us({ app: "lanyard", path: "/v1/records" }, (t) => {
    nn.log("raw lanyard sub event", t), e({ type: "Default" });
  });
}
async function QU(e) {
  var c;
  const t = Math.floor(Math.random() * 1e6), n = Wt("uv", BigInt(t)), r = [
    null,
    [null, t],
...
async function eM(e, t = null, n = null) {
  var r, s;
  try {
    const o = await xB(e, n), u = (t == null ? void 0 : t.replaceAll(".", "")) ?? "0x0", c = BigInt(u), d = Math.floor(Math.random() * 1e6), f = Wt("uv", BigInt(d)), m = [
      null,
      [null, d],
      "whose-bulk",
      c,
    ...
    try {
      ...
      nn.trackEvent(Ke.ErrorContactMatching, {
        error: E,
    ...
    try {
      const E = await w;
      if (nn.log("discoverContacts: queryResponse", E), E && E.query.nonce === f) {
        if (E.query.result === "rate limited")
          return nn.trackEvent(Ke.ErrorContactMatching, {
            error: "rate limited",
            context: "discoverContacts",
      ...
      throw nn.error("error in discoverContacts", E), nn.trackEvent(Ke.ErrorContactMatching, {
        context: "discoverContacts",
        error: E,
        errorMessage: E.message
      }), E;
    ...
    throw nn.error("error discovering contacts", o), o;
  }
}
async function xB(e, t) {
  if (nn.log("diffContactBook: last phone contact set", t), t) {
    const n = JSON.parse(t);
    nn.log("diffContactBook: lastSet", n);
    const r = e.filter((o) => !n.includes(o));
    nn.log("diffContactBook: diff", r), nn.trackEvent(Ke.DebugContactMatching, {
      context: "diffContactBook",
      diffSetLength: r.length
    });
    const s = n.filter((o) => !e.includes(o));
    return nn.log("diffContactBook: delSet", s), nn.trackEvent(Ke.DebugContactMatching, {
      context: "diffContactBook",
      delSetLength: s.length
    }), r.length === 0 && s.length === 0 ? (nn.log("diffContactBook: no changes, returning empty with lastSet"), {
      last: Wr(n),
      add: Wr([]),
      del: Wr([])
    }) : (nn.log("diffContactBook: returning diff and del", r, s), {
      last: Wr(n),
      add: Wr(r),
      del: Wr(s)
    });
  }
  return nn.log("diffContactBook: no last set, returning with add"), {
    last: Wr([]),
    add: Wr(e),
    del: Wr([])
  };
...
async function yM(e) {
  try {
    const t = Wt("uw", Y.fromCord(e).number);
...
async function wM(e) {
  const t = await W3(), n = Wt("da", ur.fromUnix(e)), r = await Ve({
    app: "groups-ui",
    path: `/v7/changes/${n}`
...
(function(e) {
  ...
  function An(g, h) {
    if (typeof g != "number")
      throw new Kt.ERR_INVALID_ARG_TYPE(h, "number", g);
  }
  function Ct(g, h, p) {
    throw Math.floor(g) !== g ? (An(g, p), new Kt.ERR_OUT_OF_RANGE("offset", "an integer", g)) : h < 0 ? new Kt.ERR_BUFFER_OUT_OF_BOUNDS() : new Kt.ERR_OUT_OF_RANGE(
      "offset",
      `>= 0 and <= ${h}`,
      g
    );
