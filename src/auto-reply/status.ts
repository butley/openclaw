...
import { formatStatusCard } from "./status-card-format.js";
...
export function buildStatusMessage(args: StatusArgs): string {
  ...
  // [FORK-PATCH-8] Status Card — formatting extracted to status-card-format.ts
  const providerLabel = (() => {
  ...
  return formatStatusCard({
    ...
    subagentsLine: args.subagentsLine,
    optionsLine,
    voiceLine,
    groupActivationValue,
    queueMode,
    queueDetails,
  });
}
