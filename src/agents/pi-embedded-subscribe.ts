  ...
  const emitToolEndSummary = (
    ...
    if (name === "edit") {
      try {
        ...
        if (a) {
          const rawOld = a.old_string ?? a.oldText ?? "";
          const rawNew = a.new_string ?? a.newText ?? "";
          const oldStr = typeof rawOld === "string" ? rawOld : JSON.stringify(rawOld);
          const newStr = typeof rawNew === "string" ? rawNew : JSON.stringify(rawNew);
          if (oldStr || newStr) {
      ...
      try {
        ...
        if (parsed) {
          const provider = typeof parsed.provider === "string" ? parsed.provider : "local";
