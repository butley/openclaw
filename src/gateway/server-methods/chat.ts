...
// [FORK-PATCH-23] Chat.send Internal Routing
import {
    ...
    if (normalizedAttachments.length > 0) {
      try {
        ...
        context.logGateway?.info?.(
          `[chat.send] attachments=${normalizedAttachments.length} images=${parsedImages.length}`,
        );
        ...
        for (const img of parsedImages) {
          try {
            ...
            context.logGateway?.warn?.(
              `[chat.send] Failed to save inbound image: ${imgErr instanceof Error ? imgErr.message : String(imgErr)}`,
            );
          }
        }
