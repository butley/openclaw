...
export const chatHandlers: GatewayRequestHandlers = {
  ...
  "chat.send": async ({ params, respond, context, client }) => {
    ...
    try {
      ...
      // [FORK-PATCH-23] Use upstream route resolver but ensure INTERNAL_MESSAGE_CHANNEL
      // is the default for webchat-originated messages. This preserves WA delivery
      // (the agent pipeline uses session entry routing, not OriginatingChannel).
      const resolvedRoute = resolveChatSendOriginatingRoute({
        client: clientInfo,
        deliver: p.deliver,
        entry,
      ...
      const ctx: MsgContext = {
        ...
        // [FORK-PATCH-23] Route resolved via resolveChatSendOriginatingRoute above.
        // For webchat clients without deliver:true, this defaults to INTERNAL_MESSAGE_CHANNEL.
