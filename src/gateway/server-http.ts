    ...
    try {
      ...
      const requestStages: GatewayHttpRequestStage[] = [
        ...
        // [FORK-PATCH-19] Gateway Media Endpoint — serves /media/* files (TTS audio, generated images) over HTTP. See patches/README.md #19.
        {
          name: "media",
          run: async () => {
            // Serve generated media files via GET /media/{filename}
            if (req.method !== "GET" || !requestPath.startsWith("/media/")) {
              return false;
            }
            // Allow web UIs on different origins (e.g. staging.hyperindiv.com) to fetch media
            // generated by this gateway host.
            ...
            if (filename && /^[\w.-]+$/.test(filename)) {
              ...
              // 3) ~/.openclaw/media/inbound/<filename> (user-sent images saved on ingest)
              candidatePaths.push(path.join(os.homedir(), ".openclaw", "media", "inbound", filename));

              // 4) /root/clawd/<filename> (legacy image output location)
