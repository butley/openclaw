...
function sseWrite(res: ServerResponse, data: Record<string, unknown>): boolean {
  if (res.writableEnded || res.destroyed) {
    console.warn(
      `[sse] sseWrite skipped: writableEnded=${res.writableEnded} destroyed=${res.destroyed} type=${String(data.type)}`,
    );
    return false;
  }
  try {
    ...
    console.warn(`[sse] sseWrite error: type=${String(data.type)} err=${String(err)}`);
    return false;
  }
}
...
export function handleSseStream(req: IncomingMessage, res: ServerResponse): boolean {
  ...
  function onChatEvent(payload: ChatEventPayload) {
    ...
    sseEventCount++;
    const payloadSessionKey = (payload as unknown as Record<string, unknown>).sessionKey as
      | string
      | undefined;
  ...
  const normalizeSessionKey = (key: string) => key.replace(/:direct:/, ":dm:");
