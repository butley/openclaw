...
export function createAgentEventHandler({
  ...
  return (evt: AgentEventPayload) => {
    ...
    if (isToolEvent) {
      ...
      // [FORK-PATCH-16/32] Flush pending assistant text before tool-start events so clients can
      ...
      // Broadcast tool events to registered WS recipients with tool-events capability.
    ...
    if (isControlUiVisible && sessionKey) {
      ...
      if (!isAborted && evt.stream === "assistant" && typeof evt.data?.text === "string") {
        ...
        // [FORK-PATCH-32] SSE Rate Limit Fix — retryable errors (429/overload) should NOT
        // finalize the run. The gateway will retry, and the SSE stream should stay open.
        // Without this, clearAgentRunContext removes the mapping and subsequent deltas are dropped.
        if (lifecyclePhase === "error") {
          const errText = typeof evt.data?.error === "string" ? evt.data.error : "";
          const isRetryable = /rate.limit|overloaded|429|503|too many|retry/i.test(errText);
          if (isRetryable) {
            return; // Skip finalization — run stays open for retry
          }
        }
        const evtStopReason =
