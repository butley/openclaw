import { A as getFlagValue, F as hasHelpOrVersion, M as getPrimaryCommand, N as getVerboseFlag, O as getCommandPathWithRootOptions, P as hasFlag, V as isValueToken, j as getPositiveIntFlagValue, k as getCommandPositionalsWithRootOptions } from "./globals-1g9PhnTZ.js";
import "./paths-dQ_clcF4.js";
import "./theme-H80Q3Qtv.js";
import { p as defaultRuntime, r as enableConsoleCapture } from "./subsystem-CVaAzJWY.js";
...
import { A_ as loadDotEnv, yg as installUnhandledRejectionHandler } from "./auth-profiles-C_hWnnsG.js";
...
import { i as formatUncaughtError } from "./errors-D5INWvGS.js";
...
import { t as emitCliBanner } from "./banner-K5da-Ns2.js";
import "./doctor-config-flow-D_Bcciy_.js";
import { n as ensureConfigReady } from "./config-guard-D_a8rZ6m.js";
...
const routeStatus = {
	...
	run: async (argv) => {
		...
		const timeoutMs = getPositiveIntFlagValue(argv, "--timeout");
		if (timeoutMs === null) return false;
		const { statusCommand } = await import("./status-BEwADrQw.js").then((n) => n.t);
		await statusCommand({
			json,
			deep,
			all,
...
const routeSessions = {
	...
	run: async (argv) => {
		...
		const { sessionsCommand } = await import("./sessions-DhS5eZcy.js");
...
const routeAgentsList = {
	...
	run: async (argv) => {
		...
		const { agentsListCommand } = await import("./agents-DzNc2xAd.js");
...
const routeMemoryStatus = {
	...
	run: async (argv) => {
		...
		const { runMemoryStatus } = await import("./auth-profiles-C_hWnnsG.js").then((n) => n.V);
...
const routes = [
	...
	routeMemoryStatus,
	{
		...
		run: async (argv) => {
			...
			const json = hasFlag(argv, "--json");
			const { runConfigGet } = await import("./config-cli-Dsvr6BUO.js");
			await runConfigGet({
				path: pathArg,
				json
			});
	...
	},
	{
		...
		run: async (argv) => {
			...
			const pathArg = positionals[0];
			if (!pathArg) return false;
			const { runConfigUnset } = await import("./config-cli-Dsvr6BUO.js");
	...
	},
	{
		...
		run: async (argv) => {
			...
			const { modelsListCommand } = await import("./models-BWh1B_Cc.js");
	...
	},
	{
		...
		run: async (argv) => {
			...
			const { modelsStatusCommand } = await import("./models-BWh1B_Cc.js");
...
async function closeCliMemoryManagers() {
	try {
		const { closeAllMemorySearchManagers } = await import("./search-manager-OoPMhEwb.js").then((n) => n.n);
...
async function runCli(argv = process$1.argv) {
	...
	try {
		...
		const { buildProgram } = await import("./program-CHtXjUNu.js");
		...
		if (primary) {
			const { getProgramContext } = await import("./program-context-BckOwVqZ.js").then((n) => n.n);
			const ctx = getProgramContext(program);
			if (ctx) {
				const { registerCoreCliByName } = await import("./command-registry-CepLIywJ.js").then((n) => n.t);
				await registerCoreCliByName(program, ctx, primary, parseArgv);
			}
			const { registerSubCliByName } = await import("./register.subclis-DzVa6ZWB.js").then((n) => n.a);
		...
		if (!shouldSkipPluginCommandRegistration({
			...
			const { loadValidatedConfigForPluginRegistration } = await import("./register.subclis-DzVa6ZWB.js").then((n) => n.a);
