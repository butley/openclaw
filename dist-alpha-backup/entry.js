#!/usr/bin/env node
import { F as hasHelpOrVersion, L as isRootHelpInvocation, R as isRootVersionInvocation } from "./globals-1g9PhnTZ.js";
...
if (!isMainModule({
	...
	function tryHandleRootVersionFastPath(argv) {
		...
		Promise.all([import("./version-BlUhcaFh.js").then((n) => n.r), import("./git-commit-BQ-mLDWs.js").then((n) => n.t)]).then(([{ VERSION }, { resolveCommitHash }]) => {
	...
	function tryHandleRootHelpFastPath(argv) {
		...
		import("./program-CHtXjUNu.js").then(({ buildProgram }) => {
	...
	if (!ensureExperimentalWarningSuppressed()) {
		...
		if (!tryHandleRootVersionFastPath(process$1.argv) && !tryHandleRootHelpFastPath(process$1.argv)) import("./run-main-h5CJQP66.js").then(({ runCli }) => runCli(process$1.argv)).catch((error) => {
