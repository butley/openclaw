import { R as resolveStorePath, lt as getDefaultRedactPatterns } from "./query-expansion-C3Ync4Cm.js";
import "./paths-BwJ6yG6k.js";
import { i as logVerbose, p as getChildLogger, s as shouldLogVerbose } from "./globals-zsOvxm3k.js";
import { a as defaultRuntime, t as createSubsystemLogger } from "./subsystem-C0UNPQiX.js";
import { F as DEFAULT_MAIN_KEY, I as buildAgentMainSessionKey, R as buildGroupHistoryKey, V as normalizeAgentId } from "./workspace-Btxyh5AG.js";
import { d as normalizeE164, l as isSelfChatMode, m as resolveJidToE164, n as clamp, u as jidToE164, y as sleep } from "./utils-DY-S31jy.js";
import "./logger-hWznDPaF.js";
import { $ as normalizeMentionText, $n as loadSessionStore, A as shouldAckReactionForWhatsApp, An as resolveTextChunkLimit, C as buildHistoryContextFromEntries, Cn as loadWebMedia, D as resolveDmGroupAccessWithLists, Dr as readWebSelfId, Dt as computeBackoff, E as resolveDmGroupAccessWithCommandGate, F as formatInboundEnvelope, Fr as resolveGroupSessionKey, Ft as toLocationContext, Gt as resolveOpenProviderRuntimeGroupPolicy, I as getReplyFromConfig, It as buildAgentSessionKey, J as finalizeInboundContext, Kt as warnMissingProviderGroupPolicyFallbackOnce, Lt as deriveLastRoutePolicy, Mn as resolveMessagePrefix, Mr as resolveChannelGroupPolicy, Mt as parseActivationCommand, N as createInboundDebouncer, Nr as resolveChannelGroupRequireMention, Nt as upsertChannelPairingRequest, O as resolvePinnedMainDmOwnerFromAllowlist, On as chunkMarkdownTextWithMode, Ot as sleepWithAbort, P as resolveInboundDebounceMs, Pt as formatLocationText, Q as buildMentionRegexes, Rt as resolveAgentRoute, S as createReplyPrefixOptions, Si as formatCliCommand, Sr as logWebSelfId, T as readStoreAllowFromForDmPolicy, Ti as normalizeStreamLevel, Tr as pickWebChannel, W as convertMarkdownTables, Wt as resolveDefaultGroupPolicy, X as hasControlCommand, Yn as getAgentScopedMediaLocalRoots, Z as shouldComputeCommandAuthorized, _i as loadConfig, b as resolveMentionGating, br as WA_WEB_AUTH_DIR, er as recordSessionMetaFromInbound, g as resolveInboundSessionEnvelopeContext, in as enqueueSystemEvent, j as dispatchReplyWithBufferedBlockDispatcher, jn as resolveIdentityNamePrefix, jr as webAuthExists, jt as normalizeGroupActivation, kn as resolveChunkMode, kt as createDedupeCache, p as setActiveWebListener, qn as registerUnhandledRejectionHandler, ri as saveMediaBuffer, rn as formatDurationPrecise, tr as updateLastRoute, v as createConnectedChannelStatusPatch, vr as resolveWhatsAppAccount, w as recordPendingHistoryEntryIfEnabled, wn as resolveMarkdownTableMode, xr as getWebAuthAgeMs, y as issuePairingChallenge, yr as resolveWhatsAppMediaMaxBytes, zt as resolveInboundLastRouteSessionKey } from "./model-selection-CEVVlPA7.js";
...
import { i as markdownToWhatsApp, r as sendReactionWhatsApp, t as sendMessageWhatsApp } from "./outbound-KTcjuEWT.js";
import "./brazil-jid-resolver-DHtuKccz.js";
import { i as waitForWaConnection, n as formatError, r as getStatusCode, t as createWaSocket } from "./session-DmjCamWh.js";
import { i as noteContactName, r as loadContactNameCache, t as createWebSendApi } from "./send-api-DB6eqqGO.js";
...
async function monitorWebInbox(options) {
	...
	const enqueueInboundMessage = async (msg, inbound, enriched) => {
		...
		const reply = async (text) => {
			const { processOutboundMentions } = await import("./send-api-DB6eqqGO.js").then((n) => n.n);
			const processed = processOutboundMentions(text);
			const payload = processed.mentions.length > 0 ? {
				text: processed.text,
		...
		noteContactName(inbound.senderE164 ?? inbound.from, senderName);
		inboundLogger.info({
			from: inbound.from,
			to: selfE164 ?? "me",
			body: enriched.body,
	...
	return {
		...
		onWhatsApp: async (jid) => await sock.onWhatsApp(jid) ?? [],
		...createWebSendApi({
			sock: {
...
//#endregion
//#region src/web/auto-reply/wa-verbose-utils.ts
/**
...
*/
function logToolNarrationDelivered(fromDisplay, hasMedia) {
	whatsappOutboundLog.debug(`[narration-delivered] to=${fromDisplay}${hasMedia ? " (media)" : ""}`);
}
//#endregion
//#region src/web/auto-reply/wa-streaming-utils.ts
/**
...
function resolveStreamDelayMs(streamLevel, charCount) {
	switch (streamLevel) {
		case "fast": return Math.max(2e3, Math.min(6e3, charCount * 20));
		case "on": return Math.max(4e3, Math.min(1e4, charCount * 40));
		case "slow": return Math.max(6e3, Math.min(15e3, charCount * 70));
		case "off": return 0;
		default:
			if (streamLevel.startsWith("custom:")) {
				const msPerChar = parseInt(streamLevel.slice(7), 10);
				if (!isNaN(msPerChar) && msPerChar > 0) return Math.max(1e3, Math.min(2e4, charCount * msPerChar));
			}
			return 0;
	}
}
/** Read the /str stream level for a session from the session store. */
function readSessionStreamLevel(sessionKey, storePath) {
	try {
		const raw = loadSessionStore(storePath)[sessionKey]?.streamLevel;
		if (!raw) return "off";
			...
			if (index > 0 && params.streamDelayMs) {
				...
				if (delayMs > 0) {
					...
					while (elapsed < delayMs) {
						const step = Math.min(3e3, delayMs - elapsed);
						await sleep(step);
						elapsed += step;
						if (elapsed < delayMs) await msg.sendComposing?.();
					}
				}
			}
...
async function processMessage(params) {
	...
	resolveChunkMode(params.cfg, "whatsapp", params.route.accountId);
	...
	const effectiveChunkMode = sessionStreamLevel === "off" ? "length" : "newline";
	let prevBlockText = null;
	const { queuedFinal } = await dispatchReplyWithBufferedBlockDispatcher({
		...
		dispatcherOptions: {
			...
			deliver: async (payload, info) => {
				if (info.kind === "block" && sessionStreamLevel === "off") return;
				if (info.kind === "block" && prevBlockText !== null) {
					const readDelayMs = resolveStreamDelayMs(sessionStreamLevel, prevBlockText.length);
					if (readDelayMs > 0) {
						params.msg.sendComposing?.();
						let elapsed = 0;
						while (elapsed < readDelayMs) {
							const step = Math.min(3e3, readDelayMs - elapsed);
							await sleep(step);
							elapsed += step;
							if (elapsed < readDelayMs) params.msg.sendComposing?.();
						}
					}
				}
				if (info.kind === "tool") {
					if (payload.text) {
						whatsappOutboundLog.debug(`[tool-narration-raw] ${payload.text.slice(0, 300)}`);
				...
				if (info.kind !== "final" && info.kind !== "block" && info.kind !== "tool") return;
				await deliverWebReply({
					...
					streamDelayMs: sessionStreamLevel !== "off" ? (chars) => resolveStreamDelayMs(sessionStreamLevel, chars) : void 0,
				...
				didSendReply = true;
				if (info.kind === "block" && payload.text) prevBlockText = payload.text;
				...
				if (info.kind === "tool") logToolNarrationDelivered(fromDisplay, hasMedia);
				else {
	...
	if (queuedFinal) await params.msg.sendAvailable?.();
	if (!queuedFinal) {
