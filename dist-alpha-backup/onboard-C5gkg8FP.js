...
import { m as restoreTerminalState, p as defaultRuntime } from "./subsystem-CVaAzJWY.js";
import { Dt as randomToken, Mt as waitForGatewayReachable, Ot as resolveControlUiLinks, Pg as readConfigFileSnapshot, Rg as writeConfigFile, St as normalizeGatewayTokenInput, _t as ensureWorkspaceAndSessions, bt as handleReset, mt as applyWizardMetadata, pt as DEFAULT_WORKSPACE } from "./auth-profiles-C_hWnnsG.js";
...
import { jt as resolveDefaultSecretProviderAlias } from "./plugins-CVp4JUox.js";
import { t as WizardCancelledError } from "./prompts-D1_edbkK.js";
import { t as createClackPrompter } from "./clack-prompter-BaXZWh7W.js";
import { t as assertSupportedRuntime } from "./runtime-guard-xFyJF4MQ.js";
...
import { i as normalizeLegacyOnboardAuthChoice, r as isDeprecatedAuthChoice, t as ONBOARD_PROVIDER_AUTH_FLAGS } from "./onboard-provider-auth-flags-nAKoluQa.js";
import { t as applyOnboardingLocalWorkspaceConfig } from "./onboard-config-D6ACtX6E.js";
...
async function collectGatewayHealthFailureDiagnostics() {
	const diagnostics = {};
	try {
		const { resolveGatewayService } = await import("./service-BVqHJOKW.js").then((n) => n.r);
	...
	try {
		const { readLastGatewayErrorLine } = await import("./diagnostics-BPlcXCmS.js").then((n) => n.t);
...
async function runNonInteractiveOnboardingLocal(params) {
	...
	if (authChoice !== "skip") {
		const { applyNonInteractiveAuthChoice } = await import("./auth-choice-D_EXDaOg.js");
	...
	if (opts.installDaemon) {
		const { installGatewayDaemonNonInteractive } = await import("./daemon-install-Mz29sAX4.js");
	...
	if (!opts.skipHealth) {
		const { healthCommand } = await import("./health-SAgL9NoO.js").then((n) => n.i);
