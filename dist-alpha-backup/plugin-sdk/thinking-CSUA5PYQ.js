...
function normalizeVerboseLevel(raw) {
	if (raw && [
		...
		"narration",
		"narrate"
	].includes(raw.toLowerCase())) return "light";
	return normalizeOnOffFullLevel(raw);
}
function normalizeStreamLevel(raw) {
	if (!raw) return;
	const key = raw.toLowerCase();
	if ([
		"off",
	...
	if ([
		...
		"quick",
		"rapid"
	].includes(key)) return "fast";
	if (["slow", "relaxed"].includes(key)) return "slow";
	if ([
		"on",
		"true",
		"yes",
	...
	if (Number.isFinite(num) && num > 0 && num <= 200) return `custom:${num}`;
}
...
//#endregion
export { normalizeFastMode as a, normalizeThinkLevel as c, resolveResponseUsageMode as d, resolveThinkingDefaultForModel as f, normalizeElevatedLevel as i, normalizeUsageDisplay as l, formatXHighModelHint as n, normalizeReasoningLevel as o, supportsXHighThinking as p, listThinkingLevels as r, normalizeStreamLevel as s, formatThinkingLevels as t, normalizeVerboseLevel as u };
