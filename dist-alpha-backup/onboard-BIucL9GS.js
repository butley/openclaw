...
import { At as randomToken, Ct as handleReset, Ft as waitForGatewayReachable, Tt as normalizeGatewayTokenInput, __ as writeConfigFile, _t as applyWizardMetadata, bt as ensureWorkspaceAndSessions, gt as DEFAULT_WORKSPACE, jt as resolveControlUiLinks, p_ as readConfigFileSnapshot } from "./reply-BRIrPGWy.js";
import { m as restoreTerminalState, p as defaultRuntime } from "./subsystem-Du2dCfuD.js";
...
import { jt as resolveDefaultSecretProviderAlias } from "./plugins-B3EU9SbO.js";
...
import "./daemon-runtime-C4_-bMd5.js";
...
import { t as applyOnboardingLocalWorkspaceConfig } from "./onboard-config-CHNcqPN5.js";
...
async function collectGatewayHealthFailureDiagnostics() {
	const diagnostics = {};
	try {
		const { resolveGatewayService } = await import("./service-DUsMTOWn.js").then((n) => n.r);
	...
	try {
		const { readLastGatewayErrorLine } = await import("./diagnostics-CdUIEgd7.js").then((n) => n.t);
...
async function runNonInteractiveOnboardingLocal(params) {
	...
	if (authChoice !== "skip") {
		const { applyNonInteractiveAuthChoice } = await import("./auth-choice-Cu74_AgP.js");
	...
	if (opts.installDaemon) {
		const { installGatewayDaemonNonInteractive } = await import("./daemon-install-Bl1Y4x_A.js");
	...
	if (!opts.skipHealth) {
		const { healthCommand } = await import("./health-BkmFV-XX.js").then((n) => n.i);
