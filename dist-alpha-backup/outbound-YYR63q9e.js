...
import { d as getChildLogger } from "./globals-1g9PhnTZ.js";
import { t as createSubsystemLogger } from "./subsystem-CVaAzJWY.js";
import { Fd as resolveMarkdownTableMode, In as convertMarkdownTables, Pd as loadWebMedia, Q as requireActiveWebListener, ao as normalizePollInput, co as redactIdentifier, jg as loadConfig } from "./auth-profiles-C_hWnnsG.js";
import { c as escapeRegExp, w as toWhatsappJid } from "./utils-hRMQE3lI.js";
import { G as resolveWhatsAppMediaMaxBytes, U as resolveWhatsAppAccount } from "./plugins-CVp4JUox.js";
import { o as generateSecureUuid } from "./delivery-queue-Dqzoxn1f.js";
import { t as resolveBrazilianJid } from "./brazil-jid-resolver-B1MenYJ2.js";
...
async function resolveJidWithBrazil(active, to) {
	const jid = toWhatsappJid(to);
	if (!active.onWhatsApp) return jid;
	try {
		const resolved = await resolveBrazilianJid({ onWhatsApp: active.onWhatsApp }, jid);
		if (resolved !== jid) outboundLog.info(`[brazil-jid] Resolved ${jid} -> ${resolved}`);
		return resolved;
	} catch (err) {
		outboundLog.warn(`[brazil-jid] Resolution failed: ${err instanceof Error ? err.message : String(err)}`);
		return jid;
	}
}
...
//#endregion
export { markdownToWhatsApp as a, sendReactionWhatsApp as i, sendMessageWhatsApp as n, sendPollWhatsApp as r, outbound_exports as t };
