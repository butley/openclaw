import { t as CONFIG_DIR } from "./utils-B2AOQkdJ.js";
...
function loadCache() {
	...
	try {
		if (fsSync.existsSync(CACHE_FILE)) jidCache = JSON.parse(fsSync.readFileSync(CACHE_FILE, "utf-8")).entries || {};
		else jidCache = {};
	} catch {
		jidCache = {};
	}
...
function saveCache() {
	try {
		fsSync.mkdirSync(path.dirname(CACHE_FILE), { recursive: true });
