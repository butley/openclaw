...
import { s as shouldLogSubsystemToConsole, t as createSubsystemLogger } from "./subsystem-JJ8bgFw2.js";
import { $r as sniffMimeFromBase64, Af as estimateBase64DecodedBytes, hu as loadOrCreateDeviceIdentity, nm as readJsonFile, rm as writeJsonAtomic, tm as createAsyncLock, vu as signDevicePayload } from "./auth-profiles-C0kh5mIB.js";
import { E as parseAgentSessionKey } from "./session-key-51LnISpq.js";
import { n as redactSensitiveText, t as getDefaultRedactPatterns } from "./redact-gt8ZHLqM.js";
...
async function extractAudioAttachments(attachments, opts) {
	const maxBytes = opts?.maxBytes ?? 2e7;
	const log = opts?.log;
	if (!attachments || attachments.length === 0) return [];
	const audio = [];
	for (const [idx, att] of attachments.entries()) {
		...
		const providedMime = normalizeMime(mime);
		const sniffedMime = normalizeMime(await sniffMimeFromBase64(b64));
		const isAudioVideoAlias = providedMime && isAudioMime(providedMime) && sniffedMime && sniffedMime.replace("video/", "audio/") === providedMime;
		const finalMime = isAudioVideoAlias ? providedMime : sniffedMime ?? providedMime;
		if (!finalMime || !isAudioMime(finalMime)) continue;
		if (sniffedMime && providedMime && sniffedMime !== providedMime && !isAudioVideoAlias) log?.warn(`attachment ${label}: mime mismatch (${providedMime} -> ${sniffedMime}), using sniffed`);
		audio.push({
			label,
...
//#endregion
export { setGatewayWsLogStyle as _, resolveApnsAuthConfigFromEnv as a, shouldClearStoredApnsRegistration as c, extractAudioAttachments as d, parseMessageWithAttachments as f, summarizeAgentEventForWsLog as g, shouldLogWs as h, registerApnsRegistration as i, resolveApnsRelayConfigFromEnv as l, logWs as m, loadApnsRegistration as n, sendApnsAlert as o, formatForLog as p, normalizeApnsEnvironment as r, sendApnsBackgroundWake as s, clearApnsRegistrationIfCurrent as t, normalizeRpcAttachmentsToChatAttachments as u };
