...
if (!isMainModule({
	...
	function tryHandleRootVersionFastPath(argv) {
		...
		Promise.all([import("./version-DcA9ITyc.js").then((n) => n.r), import("./git-commit-DL_7v9Lf.js").then((n) => n.t)]).then(([{ VERSION }, { resolveCommitHash }]) => {
	...
	function tryHandleRootHelpFastPath(argv) {
		...
		import("./program-CGPtlKhx.js").then(({ buildProgram }) => {
	...
	if (!ensureExperimentalWarningSuppressed()) {
		...
		if (!tryHandleRootVersionFastPath(process$1.argv) && !tryHandleRootHelpFastPath(process$1.argv)) import("./run-main-StSzghM-.js").then(({ runCli }) => runCli(process$1.argv)).catch((error) => {
